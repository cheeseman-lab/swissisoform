...
Usage:
    python annotate_mane.py [--test GENE_NAME]
...
def annotate_mane_status(df, mane_transcripts):
    ...
    df['MANE'] = df.apply(
        lambda row: 'Yes' if mane_transcripts.get(row['gene_name']) == row['transcript_base'] else 'No',
        axis=1
    )
...
def main():
    parser = argparse.ArgumentParser(description='Annotate isoforms with MANE Select info')
    parser.add_argument('--test', type=str, help='Test mode: show annotation for specific gene only')
    ...
    output_csv = Path('/lab/barcheese01/mdiberna/swissisoform/results/hela/mutations/isoform_level_results_mane.csv')
    ...
    if args.test:
        ...
        display_cols = ['gene_name', 'transcript_id', 'MANE', 'transcript_notes',
                       'feature_type', 'total_mutations']
    ...
    print(f"MANE Select isoforms: {mane_count} ({100*mane_count/len(annotated_df):.1f}%)")
    print(f"\nNote categories:")
