...
class SummaryAnalyzer:
    ...
    def create_gene_summary(
        ...
        for gene in pairs_df["gene"].unique():
            ...
            gene_summary = {
                ...
                "gene": gene,
                "canonical_localization": canonical_loc,
                # Localization analysis results (changed "variants" to "isoforms")
                "total_truncating_isoforms": len(truncated_seqs),
                "truncating_isoforms_with_localization_change": truncating_changes,
                "total_missense_isoforms": len(missense_variants)
                if not mutations_df.empty
                else 0,
                "missense_isoforms_with_localization_change": missense_changes,
                "total_isoforms_with_localization_change": truncating_changes
                + missense_changes,
        ...
        # Sort by total isoforms with localization changes (prioritized targets)
        if not gene_summary_df.empty:
            gene_summary_df = gene_summary_df.sort_values(
                "total_isoforms_with_localization_change", ascending=False
            )
    ...
    def analyze_dataset(self, dataset):
        ...
        if not gene_summary_df.empty:
            ...
            # Top genes by total isoforms with localization changes
            top_genes = gene_summary_df.head(10)
            print(f"Top 10 genes by isoforms with localization changes:")
            for _, gene in top_genes.iterrows():
                print(
                    f"  {gene['gene']}: {gene['total_isoforms_with_localization_change']} isoforms with loc changes "
                    f"({gene['truncating_isoforms_with_localization_change']} truncating, "
                    f"{gene['missense_isoforms_with_localization_change']} missense) | "
            ...
            total_genes_with_changes = len(
                gene_summary_df[
                    gene_summary_df["total_isoforms_with_localization_change"] > 0
                ]
            )
            ...
            print(
                f"\nSummary: {total_genes_with_changes} out of {total_genes} genes have isoforms with localization changes"
            )
            print(
