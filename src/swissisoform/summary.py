...
class SummaryAnalyzer:
    ...
    def dataset_has_data(self, dataset):
        ...
        mutation_gene_file = Path(f"../results/{dataset}/mutations/gene_level_results.csv")
        
        # Check for localization data  
        loc_files = [
            Path(f"../results/{dataset}/localization/protein_sequences_pairs_Accurate_results.csv"),
            Path(f"../results/{dataset}/localization/protein_sequences_pairs_Fast_results.csv"),
        ]
                    ...
                    for gene in mutations_df["gene"].unique():
                        ...
                        if not canonical_pairs.empty:
                            ...
                            for _, mut_seq in missense_variants.iterrows():
                                ...
                                if self.are_localizations_different(canonical_loc, mut_loc):
                                    change_record = {
                                        "dataset": dataset,
                                        "gene": gene,
    ...
    def create_gene_summary(self, dataset, loc_results, protein_data, pair_results=None):
        ...
        for gene in pairs_df["gene"].unique():
            ...
            for _, trunc_seq in truncated_seqs.iterrows():
                if self.are_localizations_different(canonical_loc, trunc_seq["prediction"]):
            ...
            if not mutations_df.empty:
                ...
                for _, mut_seq in missense_variants.iterrows():
                    if self.are_localizations_different(canonical_loc, mut_seq["prediction"]):
                        missense_changes += 1
            ...
            frameshift_or_nonsense_variants = list(set(frameshift_variants + nonsense_variants))
            ...
            gene_summary = {
                ...
                "frameshift_or_nonsense_variant_ids": ",".join(frameshift_or_nonsense_variants) if frameshift_or_nonsense_variants else "",  # Changed from "truncating"
    ...
    def analyze_dataset(self, dataset):
        ...
        # Create gene-level summary
        gene_summary_df = self.create_gene_summary(dataset, loc_results, protein_data, pair_results)
