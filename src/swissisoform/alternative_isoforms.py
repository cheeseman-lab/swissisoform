...
class AlternativeIsoform:
    ...
    def _regions_overlap(self, region1: Tuple[int, int], region2: Tuple[int, int]) -> bool:
    ...
    def _find_overlapping_groups(self, regions: List[Tuple[int, int, int]]) -> List[List[int]]:
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                self._debug_print(f"Overlap check: Region {i} {region1} vs Region {j} {region2} = {overlap}")
    ...
    def _generate_isoform_tracks(self, gene_truncations: pd.DataFrame) -> List[Dict]:
        ...
        for trunc in truncations:
            ...
            if coord_key not in seen_coordinates:
                ...
                self._debug_print(f"Removing duplicate truncation: {trunc['start']}-{trunc['end']}")
        
        if len(unique_truncations) != len(truncations):
            self._debug_print(f"Removed {len(truncations) - len(unique_truncations)} duplicate truncations")
        ...
        self._debug_print(f"Input truncations: {[(t['start'], t['end'], t['end']-t['start']) for t in truncations]}")
        ...
        regions = [(trunc["start"], trunc["end"], i) for i, trunc in enumerate(truncations)]
        ...
        if not has_overlaps:
            ...
            self._debug_print(f"Created single merged track: {leftmost_pos}-{rightmost_pos}")
            return [track]
        ...
        for group_idx, group_indices in enumerate(overlapping_groups):
            ...
            self._debug_print(f"Group truncations: {[(t['start'], t['end'], t['end']-t['start']) for t in group_truncations]}")
            
            # Check if truncations diverge on the correct side
            track_variants = self._process_overlapping_group(group_truncations, truncations, strand, gene_name)
            all_tracks.extend(track_variants)
        
        # If we didn't generate any tracks from overlapping groups, fall back to simple case
        if not all_tracks:
            self._debug_print("No valid tracks from overlapping groups, falling back to simple case")
            if strand == "+":
                truncations.sort(key=lambda x: x["start"])
            else:
                truncations.sort(key=lambda x: -x["end"])
    ...
    def _process_overlapping_group(self, group_truncations: List[Dict], all_truncations: List[Dict], 
                                 strand: str, gene_name: str) -> List[Dict]:
        ...
        """
        self._debug_print(f"Processing {len(group_truncations)} overlapping truncations")
        ...
        self._debug_print(f"Unique start positions: {unique_starts}, Unique end positions: {unique_ends}")
        
        if strand == "-":
            # For negative strand, check if they diverge on the left (start positions)
            if unique_starts <= 1:
                self._debug_print("ERROR: Overlapping truncations don't diverge on left side (negative strand)")
                self._debug_print("This appears to be an error in the BED file - treating as single truncation")
                # Keep the longest truncation
                longest = max(group_truncations, key=lambda x: x["end"] - x["start"])
                return self._create_simple_track([longest], all_truncations, strand, gene_name, "error_corrected")
            ...
            for i, trunc_i in enumerate(group_truncations):
                for j, trunc_j in enumerate(group_truncations):
                    if i != j:
                        ...
                        if (trunc_i["start"] <= trunc_j["start"] and trunc_j["end"] <= trunc_i["end"]):
                            self._debug_print(f"ERROR: Truncation {trunc_j['start']}-{trunc_j['end']} is completely contained within {trunc_i['start']}-{trunc_i['end']}")
                            self._debug_print("This appears to be an error in the BED file - treating as single truncation")
                            # Keep the longest (containing) truncation
                            longest = max(group_truncations, key=lambda x: x["end"] - x["start"])
                            return self._create_simple_track([longest], all_truncations, strand, gene_name, "containment_error_corrected")
            
            self._debug_print("Valid divergence on left side for negative strand")
            divergent_positions = sorted(set(group_starts), reverse=True)  # Sort high to low for neg strand
            
        else:  # strand == "+"
            # For positive strand, check if they diverge on the right (end positions)
            if unique_ends <= 1:
                self._debug_print("ERROR: Overlapping truncations don't diverge on right side (positive strand)")
                self._debug_print("This appears to be an error in the BED file - treating as single truncation")
                # Keep the longest truncation
                longest = max(group_truncations, key=lambda x: x["end"] - x["start"])
                return self._create_simple_track([longest], all_truncations, strand, gene_name, "error_corrected")
            ...
            divergent_positions = sorted(set(group_ends))  # Sort low to high for pos strand
        ...
        if strand == "-":
            ...
            self._debug_print(f"Extreme left position: {extreme_position}, Group max start: {max(group_starts)}, At extreme: {divergence_at_extreme}")
            ...
            self._debug_print(f"Extreme right position: {extreme_position}, Group max end: {max(group_ends)}, At extreme: {divergence_at_extreme}")
        
        if divergence_at_extreme:
            self._debug_print("CASE: Divergence at extreme end - creating tracks with different end sites only")
            return self._create_different_endpoint_tracks(group_truncations, all_truncations, strand, gene_name)
        else:
            self._debug_print("CASE: Divergence at intermediate position - creating tracks with early termination")
            return self._create_early_termination_tracks(group_truncations, all_truncations, strand, gene_name)
    
    def _create_simple_track(self, selected_truncations: List[Dict], all_truncations: List[Dict], 
                           strand: str, gene_name: str, description_suffix: str = "") -> List[Dict]:
        """Create a simple track with selected truncations plus all non-overlapping ones."""
        
        # Get all non-overlapping truncations
        regions = [(trunc["start"], trunc["end"], i) for i, trunc in enumerate(all_truncations)]
        overlapping_groups = self._find_overlapping_groups(regions)
        ...
        track = {
            ...
            "description": f"Track with {len(track_truncations)} truncations" + (f" ({description_suffix})" if description_suffix else ""),
        }
        
        return [track]
    
    def _create_different_endpoint_tracks(self, group_truncations: List[Dict], all_truncations: List[Dict], 
                                        strand: str, gene_name: str) -> List[Dict]:
        ...
        for i, selected_trunc in enumerate(group_truncations):
            self._debug_print(f"Creating track {i+1} with truncation: {selected_trunc['start']}-{selected_trunc['end']}")
            
            track_truncations = [selected_trunc]
            
            # Add all non-overlapping truncations
            regions = [(trunc["start"], trunc["end"], j) for j, trunc in enumerate(all_truncations)]
            overlapping_groups = self._find_overlapping_groups(regions)
            ...
            tracks.append(track)
            self._debug_print(f"Created track: {track['track_id']} ({track_start}-{track_end})")
        
        return tracks
    
    def _create_early_termination_tracks(self, group_truncations: List[Dict], all_truncations: List[Dict], 
                                       strand: str, gene_name: str) -> List[Dict]:
        ...
        for i, selected_trunc in enumerate(group_truncations):
            self._debug_print(f"Creating track {i+1} with selected truncation: {selected_trunc['start']}-{selected_trunc['end']}")
            ...
            if selected_pos is not None:
                if strand == "-":
                    ...
                    self._debug_print(f"Negative strand: keeping truncations up to position {selected_pos}")
                else:
                    # For positive strand, remove truncations to the right (higher indices) of the selected one  
                    valid_truncations = sorted_all[:selected_pos+1]
                    self._debug_print(f"Positive strand: keeping truncations up to position {selected_pos}")
            ...
            tracks.append(track)
            self._debug_print(f"Created track: {track['track_id']} ({track_start}-{track_end}) with {len(track_truncations)} truncations")
    ...
    def get_alternative_starts(self, gene_name: Optional[str] = None) -> pd.DataFrame:
        ...
        if gene_name is not None:
            ...
            for gene in self.get_gene_list():
                gene_truncations = self.isoforms[self.isoforms["gene_name"] == gene].copy()
        ...
        for track in tracks:
            ...
            row = {
                ...
                "track_description": track["description"],
                "num_truncations": len(track["truncations"]),
                "original_truncations": track["truncations"],  # Keep reference to original data
            }
